...
public class ReservaController {
	...
	public String guardar(@Validated Reserva reserva,Model model) {
		try {
			...
			Destino des =destinoService.buscarCiudad(id);
			System.out.println("Numero boletos: "+reserva.getBoleto());
			System.out.println("Capacidad: "+bus.getCap_bus());
			if(reserva.getBoleto()<= bus.getCap_bus()) {
				int diferencia = bus.getCap_bus()-reserva.getBoleto();
				bus.setCap_bus(diferencia);
				double total = Integer.parseInt(des.getCostDest())*reserva.getBoleto();
				...
				model.addAttribute("clientes",clienteService.listar());
				model.addAttribute("destinos",destinoService.listar());
				model.addAttribute("btn","Registro Venta");
				return "ventaForm";
			}
		}catch(Exception e) {
